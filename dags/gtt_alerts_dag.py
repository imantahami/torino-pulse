...
def fetch_and_store_alerts():
    ...
    for entity in feed.entity:
        ...
        for ie in entities:
            ...
            cur.execute("""
                ...
                (alert_id, cause, effect, severity_level, header_text, description_text,
                 url, active_period_start, active_period_end,
                 informed_route_id, informed_stop_id, informed_trip_id, fetched_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                entity.id, cause, effect, severity, header, description,
                alert_url, period_start, period_end,
                route_id or None, stop_id or None, trip_id or None,
                fetched_at
            ))
            count += 1
    ...
    cur.close()
    conn.close()
    print(f"Inserted {count} alert rows")
